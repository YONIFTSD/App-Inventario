    ...
    <CCol :xs="12">
      <CCard class="mb-4">
        <CCardHeader>
            <b-row>
                <b-col md="10">
                    <strong>Modulo de Egresos | Nuevo</strong>
                </b-col>
                <b-col md="2">
                ...
                <b-row class="justify-content-center">
                    ...
                    <b-col md="4">
                        <b-form-group label="Tipo de Gasto" :description="errors.id_type_expense">
                            <b-form-select size="sm" v-model="expense.id_type_expense" :options="type_expense"></b-form-select>
                        </b-form-group> 
                    </b-col>
...
function GetCorrelative() {
    let me = this;
    let url = this.url_base + "data-manager/get-correlative-by-module/Expense"
    axios({
        method: "GET",
        url: url,
        headers: {token:this.muser.api_token, module:this.module, role:this.role},
...
function AddExpense() {
    ...
    let me = this;
    let url = this.url_base + "expenses/add";
    ...
    axios({
        ...
        if (response.data.status == 201) {
            me.expense.id_expense = '';
            me.expense.id_user = '';
            me.expense.id_type_expense = '';
            me.expense.code = '';
            me.expense.date = moment(new Date()).local().format("YYYY-MM-DD");
            me.expense.payment_method = '008';
            me.expense.coin = 'PEN';
            me.expense.observation = '';
            me.expense.exchange_rate = '1.00';
            me.expense.total = '0.00';
            me.expense.state = '1';
            Swal.fire({ icon: 'success', text: response.data.message, timer: 3000})
            me.$router.push({name: "ExpenseList" });
...
function Validate() {
    ...
    this.errors.id_type_expense = this.expense.id_type_expense.length == 0 ? 'Seleccione un gasto':'';
    ...
    if (this.errors.id_type_expense.length > 0) this.validate = true;
