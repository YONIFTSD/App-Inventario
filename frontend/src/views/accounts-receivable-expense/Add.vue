    ...
    <CCol :xs="12">
      <CCard class="mb-4">
        <CCardHeader>
            <b-row>
                <b-col md="10">
                    <strong>Modulo Egreso de Cobros | Nuevo</strong>
                </b-col>
                <b-col md="2">
                    <b-link :to="{ path: '/egreso-cobros/listar' }" class="btn btn-sm form-control btn-primary" append title="Regresar" ><font-awesome-icon icon="fa-solid fa-circle-chevron-left" /> Regresar</b-link >
                </b-col>
            </b-row>
        </CCardHeader>
        <CCardBody>
    ...
    axios({
        ...
        if (response.data.status == 200) {
           response.data.result.map((item) => {
            me.type_expense.push({value: item.description, text: item.name});
           }); 
        }
    }).catch((error) => {
        Swal.fire({ icon: 'error', text: 'A ocurrido un error', timer: 3000,})
...
function GetCorrelative() {
    let me = this;
    let url = this.url_base + "data-manager/get-correlative-by-module/AccountReceivableExpense"
    axios({
        method: "GET",
        url: url,
        headers: {token:this.muser.api_token, module:this.module, role:this.role},
...
function AddExpense() {
    ...
    let me = this;
    let url = this.url_base + "accounts-receivable-expenses/add";
    ...
    axios({
        ...
        if (response.data.status == 201) {
            ...
            me.$router.push({name: "AccountReceivableExpenseList" });
...
function Validate() {
    ...
    this.errors.description = this.expense.description.length == 0 ? 'Seleccione un gasto':'';
