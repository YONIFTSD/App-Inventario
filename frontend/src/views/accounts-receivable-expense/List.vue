    ...
    <CCol :xs="12">
      <CCard class="mb-4">
        <CCardHeader>
            <b-row>
                <b-col md="12">
                    <strong>Modulo Egreso de Cobros | Listar</strong>
                </b-col>
            </b-row>
        ...
        <CCardBody>
            <b-row>
                ...
                <b-col md="1">
                    <b-form-group v-if="Permission('AccountReceivableExpenseAdd')" label=".">
                        <b-link class="btn btn-sm form-control btn-primary" :to="{ path: '/egreso-cobros/nuevo' }" append title="Nuevo" ><font-awesome-icon icon="fa-solid fa-file-circle-plus" /></b-link >
                    </b-form-group>
                </b-col>
                            ...
                            <tr v-for="(item, it) in data_table" :key="it">
                                ...
                                <td class="text-center">
                                    <b-dropdown variant="dark" dark right size="sm" text="Acciones">
                                        <b-dropdown-item v-if="Permission('AccountReceivableExpenseEdit')" @click="PageEdit(item.id_expense)"><font-awesome-icon title="Editar" icon="fa-solid fa-pen-to-square" /> Editar</b-dropdown-item>
                                        <b-dropdown-item v-if="Permission('AccountReceivableExpenseView')" @click="PageView(item.id_expense)"><font-awesome-icon title="Ver" icon="fa-solid fa-eye" /> Ver</b-dropdown-item>
                                        <b-dropdown-item v-if="Permission('AccountReceivableExpenseDelete')" @click="ConfirmDelete(item.id_expense)"><font-awesome-icon title="Eliminar" icon="fa-solid fa-trash-can" /> Eliminar</b-dropdown-item>
                                    </b-dropdown>
                                </td>
                            </tr>
                        </tbody>
...
export default {
    ...
    data() {
      return {
        module:'AccountReceivableExpense',
        role:'List',
...
function ListSettlementExpense() {
    ...
    let url = this.url_base + "accounts-receivable-expenses/list/" + this.from+"/"+this.to +"/" + search + "?page=" + this.currentPage;
    axios({
        method: "GET",
        url: url,
...
function Delete(id_expense) {
    let me = this;
    let url = this.url_base + "accounts-receivable-expenses/delete/" + id_expense;
    axios({
        method: "delete",
        url: url,
