    ...
    <CCol :xs="12">
      <CCard class="mb-4">
        <CCardHeader>
            <b-row>
                <b-col md="10">
                    <strong>Modulo Liquidación Diaria Egresos | Nuevo</strong>
                </b-col>
                <b-col md="2">
                    <b-link :to="{ path: '/liquidacion-diaria-egresos/listar' }" class="btn btn-sm form-control btn-primary" append title="Regresar" ><font-awesome-icon icon="fa-solid fa-circle-chevron-left" /> Regresar</b-link >
                </b-col>
            </b-row>
        </CCardHeader>
        <CCardBody>
...
function GetCorrelative() {
    let me = this;
    let url = this.url_base + "data-manager/get-correlative-by-module/DailySettlementExpense"
    axios({
        method: "GET",
        url: url,
        headers: {token:this.muser.api_token, module:this.module, role:this.role},
...
function AddExpense() {
    ...
    let me = this;
    let url = this.url_base + "daily-settlement-expenses/add";
    ...
    axios({
        ...
        if (response.data.status == 201) {
            ...
            me.$router.push({name: "DailySettlementExpenseList" });
